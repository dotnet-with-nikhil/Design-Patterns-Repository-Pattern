...
This guide explains the difference between `IEnumerable<T>` and `IQueryable<T>` when working with Entity Framework Core.
...
- `IQueryable<Product>`
- Deferred execution
- When the database is actually executed
- LINQ-to-Objects vs LINQ-to-Entities
- `ToList()` vs `ToListAsync()`
- Query composition
- Performance
- Practical Repository and Service examples
